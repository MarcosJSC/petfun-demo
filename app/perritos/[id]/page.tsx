...
const [
  documentosAbiertos,
  setDocumentosAbiertos,
] = useState(false);

const [
  documentos,
  setDocumentos,
] = useState<DocumentoPerrito[]>([]);
...
] = useState<DocumentoPerrito | null>(null);
...
async function cargarDocumentos(
  ...
  try {
      ...
      await Promise.all(
        (data ?? []).map(
          async (documento) => {
            ...
            if (urlError) {
              console.error(
                "Error creando URL del documento:",
                urlError
              );
            }
    ...
    setDocumentos(
      documentosConUrl as DocumentoPerrito[]
    );
    ...
    console.error(
      "Error cargando documentos del perrito:",
      error
    );
  } finally {
  ...
  try {
    ...
    archivoPath =
      `${perrito.sucursal_id}/${perrito.id}/${nombreArchivo}`;
...
async function alternarDocumentos() {
  const nuevoEstado =
    !documentosAbiertos;
...
>
  ...
  >
    <div>
      ...
      >
        Fotografías de carnés y comprobantes
      </div>
    </div>
